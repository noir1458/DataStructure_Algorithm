...
bool deleteHelper(TrieNode* root, const char* key, int depth) {
    ...
    if (*key == '\0') {
        ...
        if (root->children[index]) {
            ...
            */
            bool delChild = deleteHelper(root->children[index], key + 1, depth + 1);
            if (delChild) {
...
void suggestions(TrieNode* root, const char* currentPrefix) {
    ...
    for (int i = 0; i < ALPHABET_SIZE; i++) {
        if (root->children[i]) {
            ...
            */
            char newPrefix[100];
            strcpy_s(newPrefix,sizeof(newPrefix), currentPrefix);
            int length = strlen(currentPrefix);
